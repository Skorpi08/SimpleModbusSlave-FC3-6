...
if ((*ModbusPort).available())
  {
    ...
    if (buffer > 7) 
      ...
      if (id == slaveID) // id == slaveID || broadcastFlag if the recieved ID matches the slaveID or broadcasting id (0), continue
      {
        ...
        if (calculateCRC(buffer - 2) == crc) // if the calculated crc matches the recieved crc continue
        {
				  ...
				  else if (function == 16)
				  {
					...
					// ------------- JOAD ------------------
					// Fix... Extra byte? 180 ohm mellan GND och - (B)
					// ------------- JOAD ------------------
					  if (frame[6] == (buffer - 9) || frame[6] == (buffer - 10))
					  {
						  if (startingAddress < holdingRegsSize) // check exception 2 ILLEGAL DATA ADDRESS
						  {
							  if (maxData <= holdingRegsSize) // check exception 3 ILLEGAL DATA VALUE
